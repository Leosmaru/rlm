...
// Routers
import { router as rlmRouter } from './endpoints/rlm.js'; // RLM: звонок к провайдеру для ноды «API»
import { router as rlmSoulRouter } from './endpoints/rlm-soul.js'; // RLM: память «Душа» (MD + смысловой поиск)
import { router as rlmCardsRouter } from './endpoints/rlm-cards.js'; // RLM: каталог карточек (chub.ai / character-tavern)
...
// RLM: наш маршрут звонка к провайдеру — ДО CSRF/логина, чтобы узловой холст звал его
// простым POST. Защита остаётся whitelist'ом (только localhost). См. src/endpoints/rlm.js
app.use('/api/rlm', rlmRouter);
// RLM: маршруты памяти «Душа» — тоже ДО CSRF (холст ходит без токена). См. endpoints/rlm-soul.js
app.use('/api/rlm/soul', rlmSoulRouter);
// RLM: каталог карточек (chub.ai / character-tavern) — тоже ДО CSRF. См. endpoints/rlm-cards.js
app.use('/api/rlm/cards', rlmCardsRouter);
// RLM: доступ по сети (whitelist из приложения) — ПОСЛЕ whitelist-middleware (выше), поэтому менять
// список может только уже разрешённое устройство; ПК (127.0.0.1) разрешён всегда. См. endpoints/rlm-net.js
app.use('/api/rlm/net', rlmNetRouter);
// RLM: общее KV-хранилище (общий конфиг API на ПК и телефоне) — тоже ПОСЛЕ whitelist. См. endpoints/rlm-store.js
app.use('/api/rlm/store', rlmStoreRouter);
// RLM: перезапуск приложения (кнопка «Перезапуск» с телефона) — тоже ПОСЛЕ whitelist, чтобы перезапустить
// мог только доверенный. Просит главный процесс Electron сделать app.relaunch(). См. endpoints/rlm-app.js
app.use('/api/rlm/app', rlmAppRouter);
...
// Почему именно этот сервер, а не отдельный порт: CSP холста (app/index.html) — connect-src 'self',
// то есть fetch к /api/rlm/* разрешён только на свой origin. Раздача с 8100 = один origin, без CORS.
// Доступ ограничен тем же whitelist'ом, что и остальной сервер (проверка выше). Логина ST не требует —
// монтируем до requireLoginMiddleware, как и сами /api/rlm/*.
const rlmAppDirectory = path.join(serverDirectory, '..', 'app');
// no-store: телефон всегда берёт СВЕЖИЙ холст (index.html/renderer.js/styles.css), а не кэш — иначе
// после правок страница «застывает во времени». Приложение в активной разработке — кэш тут вреден.
app.use('/rlm', express.static(rlmAppDirectory, {
    etag: false,
    lastModified: false,
...
async function postSetupTasks(result) {
    ...
    if (cliArgs.listen) {
        console.log();
        // RLM: подсказать адрес холста для телефона в локальной сети (http://<ip-пк>:<порт>/rlm/).
        // Берём все не-внутренние IPv4 адреса машины — обычно это адрес в Wi-Fi/LAN.
        try {
            const scheme = cliArgs.ssl ? 'https' : 'http';
            const lanIPs = Object.values(os.networkInterfaces())
                .flat()
                .filter((i) => i && i.family === 'IPv4' && !i.internal)
                .map((i) => i.address);
            if (lanIPs.length) {
                console.log(color.green('RLM: открыть холст с телефона в локальной сети —'));
                for (const ip of lanIPs) {
                    console.log(color.green(`  ${scheme}://${ip}:${cliArgs.port}/rlm/`));
                }
                console.log('(телефон должен быть в списке whitelist в config.yaml — см. секцию SECURITY)');
                console.log();
            }
        } catch { /* адрес — необязательная подсказка, молча пропускаем */ }
