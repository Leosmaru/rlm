...
**Док `Status` — структура сцены (с 2026-09-21).** Модель просят вернуть строгий JSON `{location, time, present:[{name, pose, state}], departed:[], atmosphere}`; разбор трёхступенчатый (`statusParse`): JSON → список подписанных строк → обычный текст. На диск док ложится **рендером** (`LOCATION:` / `PRESENT:` с именем и его свойствами **разными строками** — слитая строка на следующем ходу читается моделью как имя и плодит второго персонажа). Из разобранного собирается строка состава сцены для переклички лорбука; разбор не удался — в неё идёт весь текст дока. Чем ответил провайдер, видно в ноде `API` («Формат ответа»).

**Доки вида `lore`.** `World` и `Topics` больше не файлы: их текст модель выдаёт блоками `ENTRY: имя` + `keys:` + содержание, а `soulWriteLoreEntries` кладёт их записями в «Лорбук чата» (имя совпало — запись переписывается, нет — заводится новая). Старые файлы тем переносятся разово: `tools/migrate-topics-to-lore.py`.
...
- `_vectors.json` — **кэш векторов** (служебный, не память): `{ хэш_текста: [числа] }`, 4 знака после запятой,
  до 1200 записей на чат. Ключ — `sha1` текста, поэтому правка текста сама «отвязывает» старый вектор.
  Файл можно удалить в любой момент — пересчитается.
- `_healing.jsonl` — **журнал правок памяти** (служебный): по строке JSON на перезапись дока,
  `{ at, file, removed[], added[], sizes[] }`. Считается сравнением предложений, без модели.
  Показывается кнопкой «🩹 Журнал правок» в ноде `Душа`.
  Оба файла начинаются с `_` и **не имеют расширения `.md`** — поэтому не попадают ни во вьюер, ни в промт.
...
- `nodes[]`: `{ type, x, y, collapsed, data }` — `type` = api / options / local / sysprompt / prompt / character / persona / lorebook / soul / director / embedder / translator / tts / chat / console / scanner; `data` = значения ноды по типу:
  ...
  - `lorebook` — `{ entries[], scan, sticky, budget, scope }`, где запись = `{ id, name, trigger, injection, keys, keys2, logic, whole, case, content, sticky, cooldown, delay, prob, order, constant, ignoreBudget }` (+ поля под тип: `semTrigger`/`semThreshold`/`vecThreshold`, `msgMin`/`msgMax`, `dependsOn`/`chainDelay`). `trigger` = keyword/always_on (знание) или range/random/chain (события Режиссёра); смысловые `semantic`/`vectorized` **вырезаны 2026-09-21** — старые записи читаются как `keyword`, их `semTrigger`/`semThreshold`/`vecThreshold` остаются в файле как легаси (сырьё для генератора ключей); `sticky` — сколько ходов запись держится после срабатывания (0 = как у книги); `injection` = lore/event; `scan` = дальность сканирования; `scope` = world/char/chat (видна в имени «Лорбук · <область>»). **`order`** — приоритет для **бюджета** (выше = раньше влезает; импорт из ST `insertion_order`); **`ignoreBudget`** — запись всегда влезает мимо лимита (импорт из `extensions.ignore_budget`). **`sticky`** — липкость книги в ходах (дефолт 3); **`budget`** — токенный бюджет ЭТОГО лорбука (пусто = авто, 25% от контекста ноды «Опции» своего конвейера; число = жёсткий лимит). Бюджет — движок `loreText`/`loreBudgetTokens`, см. `architecture.md`/`decisions.md` Р-22.
...
**Ключ/настройки `API`** — в **серверной базе** (ключ `rlm.apiConfig` = `{ provider, base, key, model, mode }`), **не в графе**; одно соединение на все устройства (см. `decisions.md` Р-30). При экспорте графа в файл ключа нет. Сборки/сессии — тоже в серверной базе: `rlm.presets`/`rlm.preset.<id>` (пресеты), `rlm.chatgraph.<id>` (снимок чата), `rlm.current` (контекст); `rlm.graph` — **легаси**. **Пресеты ноды `API`** (модель + её сэмплеры одной кнопкой) — отдельный общий ключ `rlm.apiPresets` (`[{ id, name, api:{ provider, base, model, instruct, provList, provStrict, mode }, options:{ values[], reason }, local?:{…} }]` — подпись-роль `label` и сам `preset` в снимок не идут, ключа API там нет: он живёт в `rlm.apiConfig`) + служебный `rlm.apiPresets.seeded` (два стартовых пресета сеются один раз); какой пресет выбран, помнит снимок чата — поле `preset` у ноды типа `api`. Оформление (одна сессия): `rlm-style`/`rlm-preset`/`rlm-wire2`/`rlm-wire-op`/`rlm.shade`.
...
**Варианты ПОСЛЕДНЕГО ответа модели и память фидбеков** (с 2026-09-08, только сингл/группа; Telegram не пользуется): у последнего `char`-сообщения могут быть `swipes[]` — массив **объектов-вариантов** `{ text, sources, critic, _state, _stateR, _tr, _showTr }` (не строк, как у приветствия), `swipeIdx` — какой показан, `fbs[]` — накопленные замечания пользователя (строки), `fbMode` (`hard`/`soft`) и `fbReason` — режим и рассуждение последнего окна фидбека. Поля живут в логе, пока игрок не ответил: `chatSend` → `sealVariants` стирает их все, остаётся один текст. Удаление сообщения стирает всё вместе с ним.
