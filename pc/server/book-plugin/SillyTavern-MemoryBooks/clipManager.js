...
import { getSceneMarkers, setSceneMarker, getCurrentSceneState } from './sceneManager.js';
...
export const DEFAULT_KEYWORD_PROMPT_TEMPLATE = `You are a lorebook keyword generator. Read the text below and output the keywords that should trigger this lorebook entry.

Rules:
- Output ONLY a single comma-separated list of keywords. No explanations, no numbering, no quotes, no extra text.
- Use 3 to 8 concise keywords or short phrases.
- Prefer proper nouns: names of characters, places, items, organizations, and the main topics/relationships mentioned.
- Use the same language as the text.
- Do not invent keywords that are not supported by the text.
...
let floatingClipScissorsButton = null;
let floatingSceneStartButton = null;
let floatingSceneEndButton = null;
let floatingSceneMessageId = null;
...
function isFloatingSceneButtonsEnabled() {
    return extension_settings?.STMemoryBooks?.moduleSettings?.showFloatingSceneButtons !== false;
}

function isFloatingMenuEnabled() {
    return isFloatingClipEnabled() || isFloatingSceneButtonsEnabled();
}
...
function getFloatingSelectionState() {
    if (!isFloatingMenuEnabled()) return null;
    ...
    return DOMPurify.sanitize(`
        ...
        <div class="stmb-clip-modal">
            ...
            <div id="stmb-clip-new-entry-fields" class="world_entry_form_control">
                ...
                <div id="stmb-clip-keywords-row">
                    <div class="stmb-clip-label-row">
                        <h4>${escapeHtml(tr('STMemoryBooks_Clip_Keywords', 'Keywords'))}</h4>
                        <div class="buttons_block gap5px">
                            <button id="stmb-clip-generate-keywords" type="button" class="menu_button stmb-clip-compact-btn"><i class="fa-solid fa-wand-magic-sparkles"></i> ${escapeHtml(tr('STMemoryBooks_Keywords_Generate', 'Generate'))}</button>
                            <button id="stmb-clip-edit-keyword-prompt" type="button" class="menu_button stmb-clip-compact-btn" title="${escapeHtml(tr('STMemoryBooks_Keywords_EditPrompt', 'Edit keyword prompt'))}"><i class="fa-solid fa-pen-to-square"></i></button>
                        </div>
                    </div>
...
function attachClipModalHandlers(popup, lorebookName, lorebookData, clipEntries) {
    ...
    const generateKeywordsButton = dlg.querySelector('#stmb-clip-generate-keywords');
    const editKeywordPromptButton = dlg.querySelector('#stmb-clip-edit-keyword-prompt');
    const keywordsInput = dlg.querySelector('#stmb-clip-keywords');

    generateKeywordsButton?.addEventListener('click', async () => {
        const sourceText = getBulletText().trim();
        if (!sourceText) {
            toastr.warning(tr('STMemoryBooks_Keywords_NoText', 'Add some clip text first, then generate keywords.'), 'STMemoryBooks');
            return;
        }
        const originalLabel = generateKeywordsButton.innerHTML;
        generateKeywordsButton.disabled = true;
        generateKeywordsButton.textContent = tr('STMemoryBooks_Jobs_Generating', 'Generating');
        try {
            const keywords = await requestKeywordSuggestions(sourceText);
            if (keywordsInput) {
                const existing = parseKeywords(keywordsInput.value || '');
            ...
            toastr.success(tr('STMemoryBooks_Keywords_Generated', 'Keywords generated.'), 'STMemoryBooks');
        } catch (error) {
            console.error(`${MODULE_NAME}: Keyword generation failed:`, error);
            toastr.error(error?.message || tr('STMemoryBooks_Keywords_Failed', 'Keyword generation failed.'), 'STMemoryBooks');
        } finally {
            generateKeywordsButton.disabled = false;
            generateKeywordsButton.innerHTML = originalLabel;
        }
    });

    editKeywordPromptButton?.addEventListener('click', async () => {
        await showKeywordPromptEditorPopup();
    });
...
export function hideFloatingClipButton() {
    ...
    floatingClipScissorsButton = null;
    floatingSceneStartButton = null;
    floatingSceneEndButton = null;
    floatingSceneMessageId = null;
}
...
function applyFloatingSceneButtonStates() {
    if (floatingSceneMessageId == null) return;
    const { start, end } = getCurrentSceneState();
    floatingSceneStartButton?.classList.toggle('on', start === floatingSceneMessageId);
    floatingSceneEndButton?.classList.toggle('on', end === floatingSceneMessageId);
}

function createFloatingSceneButton(extraClass, iconClass, titleKey, titleFallback, type) {
    const button = document.createElement('div');
    button.classList.add('stmb_floating_clip_button', extraClass, 'fa-solid', iconClass, 'interactable');
    button.title = tr(titleKey, titleFallback);
    ...
    button.addEventListener('click', (event) => {
        event.preventDefault();
        event.stopPropagation();
        if (floatingSceneMessageId == null) {
            hideFloatingClipButton();
            return;
        }
        setSceneMarker(floatingSceneMessageId, type);
        applyFloatingSceneButtonStates();
    });
    return button;
}
...
function createFloatingClipButton() {
    ...
    menu.classList.add('stmb_floating_clip_menu');
    ...
    if (isFloatingClipEnabled()) {
        ...
        scissors.classList.add('stmb_floating_clip_button', 'fa-solid', 'fa-scissors', 'interactable');
        scissors.title = tr('STMemoryBooks_Clip_ButtonTitle', 'Clip highlighted text to Memory Book');
        scissors.setAttribute('tabindex', '0');
        scissors.setAttribute('data-i18n', '[title]STMemoryBooks_Clip_ButtonTitle');
        scissors.addEventListener('mousedown', (event) => {
            event.preventDefault();
            event.stopPropagation();
        });
        scissors.addEventListener('click', async (event) => {
            event.preventDefault();
            event.stopPropagation();
        ...
        floatingClipScissorsButton = scissors;
    }

    if (isFloatingSceneButtonsEnabled()) {
        floatingSceneStartButton = createFloatingSceneButton(
            'stmb_floating_scene_start', 'fa-caret-right',
            'STMemoryBooks_MarkSceneStart', 'Mark Scene Start', 'start');
        floatingSceneEndButton = createFloatingSceneButton(
            'stmb_floating_scene_end', 'fa-caret-left',
            'STMemoryBooks_MarkSceneEnd', 'Mark Scene End', 'end');
        menu.appendChild(floatingSceneStartButton);
        menu.appendChild(floatingSceneEndButton);
    }
...
function updateFloatingClipButton() {
    ...
    // Track which message the selection belongs to so the scene arrows act on it.
    const mesid = parseInt(state.messageElement?.getAttribute('mesid'), 10);
    floatingSceneMessageId = Number.isFinite(mesid) ? mesid : null;
    applyFloatingSceneButtonStates();
...
function getKeywordPromptTemplate() {
    const saved = getModuleSettings().keywordPromptTemplate;
    return typeof saved === 'string' && saved.trim()
        ? saved
        : DEFAULT_KEYWORD_PROMPT_TEMPLATE;
}

function setKeywordPromptTemplate(template) {
    getModuleSettings().keywordPromptTemplate = String(template || '');
    saveSettingsDebounced();
}

function validateKeywordPromptTemplate(template) {
    const value = String(template || '');
    if (!value.trim()) {
        return tr('STMemoryBooks_PromptCannotBeEmpty', 'Prompt cannot be empty');
    }
    if (!value.includes('{{TEXT}}')) {
        return tr('STMemoryBooks_Keywords_PromptMissingText', 'The keyword prompt must include {{TEXT}}.');
    }
    return null;
}

function buildKeywordPrompt(text, template = getKeywordPromptTemplate()) {
    return String(template || DEFAULT_KEYWORD_PROMPT_TEMPLATE).replace(/\{\{TEXT\}\}/g, String(text || ''));
}

/**
 * Parse an AI keyword response into a clean array. Tolerates commas, newlines,
 * bullet points, numbering, and surrounding quotes.
 */
function parseGeneratedKeywords(text) {
    return String(text || '')
        .split(/[,\n;]+/)
        .map(keyword => keyword
            .replace(/^[\s\-*•\d.)\]]+/, '')
...
/**
 * Ask the AI to suggest activation keywords for the given text.
 * Reuses the Compaction profile connection.
 */
async function requestKeywordSuggestions(text, profileIndex = getCompactionProfileIndex()) {
    const template = getKeywordPromptTemplate();
    const templateError = validateKeywordPromptTemplate(template);
    if (templateError) {
    ...
    const stmbMaxTokens = Number.parseInt(extension_settings?.STMemoryBooks?.moduleSettings?.maxTokens, 10);
    if (Number.isFinite(stmbMaxTokens) && stmbMaxTokens > 0) {
        extra.max_tokens = Math.min(stmbMaxTokens, 256);
    } else {
        extra.max_tokens = 256;
    }

    const { text: responseText } = await requestCompletion({
        api: connection.api,
        model: connection.model || '',
        temperature: connection.temperature ?? 0.3,
        prompt: buildKeywordPrompt(text, template),
        endpoint: connection.endpoint,
        apiKey: connection.apiKey,
        reverseProxy: !!connection.reverseProxy,
        extra,
    ...
    const keywords = parseGeneratedKeywords(responseText);
    if (keywords.length === 0) {
        throw new Error(tr('STMemoryBooks_Keywords_EmptyResult', 'The AI did not return any keywords.'));
    }
    return keywords;
}

async function showKeywordPromptEditorPopup() {
    const content = DOMPurify.sanitize(`
        <h3>${escapeHtml(tr('STMemoryBooks_Keywords_PromptTitle', 'Keyword Generation Prompt'))}</h3>
        <div class="world_entry_form_control">
            <small class="opacity70p">${escapeHtml(tr('STMemoryBooks_Keywords_PromptHelp', 'Use {{TEXT}} where the clip text should be inserted. The AI must return a comma-separated keyword list.'))}</small>
            <textarea id="stmb-keyword-prompt-template" class="text_pole textarea_compact" rows="18">${escapeHtml(getKeywordPromptTemplate())}</textarea>
        </div>
        <div class="buttons_block gap10px">
            <button id="stmb-keyword-save-prompt" type="button" class="menu_button whitespacenowrap">${escapeHtml(tr('STMemoryBooks_Compaction_SavePrompt', 'Save Prompt'))}</button>
            <button id="stmb-keyword-reset-prompt" type="button" class="menu_button whitespacenowrap">${escapeHtml(tr('STMemoryBooks_Compaction_ResetPrompt', 'Reset to Default'))}</button>
            <button id="stmb-keyword-cancel-prompt" type="button" class="menu_button whitespacenowrap">${escapeHtml(tr('STMemoryBooks_Cancel', 'Cancel'))}</button>
        </div>
    `);
    ...
    markStmbPopup(popup);
    const showPromise = popup.show();
    const textarea = popup.dlg?.querySelector('#stmb-keyword-prompt-template');

    popup.dlg?.querySelector('#stmb-keyword-save-prompt')?.addEventListener('click', () => {
        const nextTemplate = textarea?.value || '';
        const error = validateKeywordPromptTemplate(nextTemplate);
        if (error) {
            toastr.error(error, 'STMemoryBooks');
            return;
        }
        setKeywordPromptTemplate(nextTemplate);
        toastr.success(tr('STMemoryBooks_Keywords_PromptSaved', 'Keyword prompt saved.'), 'STMemoryBooks');
        popup.completeAffirmative();
    });
    popup.dlg?.querySelector('#stmb-keyword-reset-prompt')?.addEventListener('click', () => {
        if (textarea) textarea.value = DEFAULT_KEYWORD_PROMPT_TEMPLATE;
    });
    popup.dlg?.querySelector('#stmb-keyword-cancel-prompt')?.addEventListener('click', () => {
