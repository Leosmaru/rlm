...
// Loopback (сам ПК / приложение) должен быть разрешён ВСЕГДА — иначе правкой из
// приложения можно случайно закрыть доступ самому себе.
const LOOPBACK = ['127.0.0.1', '::1'];

/**
 * Текущий белый список (копия — менять только через setWhitelist).
 * @returns {string[]}
 */
...
/**
 * Заменяет белый список и сразу применяет его БЕЗ перезапуска сервера
 * (middleware замкнут на этот же массив — мутируем на месте), затем
 * сохраняет в whitelist.txt, чтобы список пережил перезапуск.
 * Управляется из приложения RLM (панель «Доступ по сети»).
 * @param {string[]} entries Новый список IP/подсетей
 * @returns {string[]} Применённый (провалидированный) список
 */
export function setWhitelist(entries) {
    const validated = validateWhitelist(Array.isArray(entries) ? entries : []);
    for (const lo of LOOPBACK) {
        if (!validated.includes(lo)) validated.push(lo);
    }
    // мутируем НА МЕСТЕ (не переприсваиваем ссылку) — чтобы уже созданный middleware видел новый список
    whitelist.length = 0;
    whitelist.push(...validated);
    persistWhitelist();
    return getWhitelist();
}

/**
 * Сохраняет текущий белый список в whitelist.txt (по строке на запись).
 * Этот файл штатно перекрывает whitelist из config.yaml при следующем старте,
 * поэтому комментарии в config.yaml остаются нетронутыми.
 */
function persistWhitelist() {
    try {
        fs.writeFileSync(whitelistPath, whitelist.join('\n') + '\n', 'utf-8');
    } catch (e) {
        console.warn(`Whitelist ${color.red('Warning')}: не удалось сохранить ${color.yellow(whitelistPath)} - ${e.message}`);
    }
}
